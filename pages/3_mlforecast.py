...
if selected_stock:
    ...
    if stock_history.empty:
        ...
        st.header(f"{selected_stock} Stock Price - {selected_time} - {selected_coverage}% PIs with AutoXGBoost")
        ...
        auto_mlf = AutoMLForecast(models={'xgb': AutoXGBoost()}, freq='B', season_length=21)

        auto_mlf.fit(train, n_windows=3, h=horizon, fitted=True, num_samples=2, prediction_intervals=PredictionIntervals(method="conformal_distribution"))

        train_result = auto_mlf.forecast_fitted_values(level=[selected_coverage])
        train_result = train_result.set_index("unique_id")
        train_result = rename_forecast_columns(train_result)

        forecast_df = auto_mlf.predict(h=horizon, level=[selected_coverage])
        ...
        test_result = pd.merge(test_forecast, test, on=['unique_id', 'ds'], how='left')
